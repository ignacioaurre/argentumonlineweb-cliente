...
class Package {
    ...
    getPackageID(): ClientPacketValue {
        return this.getByte() as ClientPacketValue;
    }

    setPackageID(packageID: ServerPacketValue | number): void {
        this.bufferSnd = new ByteBuffer(ByteBuffer.DEFAULT_CAPACITY, true);
        this.writeByte(packageID);
    }

    writeByte(numByte: number | boolean, signed?: boolean): void {
        const val = numByte === true ? 1 : numByte === false ? 0 : (numByte ?? 0);
        if (signed) {
